...
/**
 * An API module of phoenix Finance contracts.
...
export class TombFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    // Uniswap V2 Pair
    this.TOMBWAVAX_LP = new Contract(externalTokens['PHOENIX-AVAX-LP'][0], IUniswapV2PairABI, provider);
  ...
  async getTombStat(): Promise<TokenStat> {
    const { PhoenixAvaxRewardPool } = this.contracts;
    const supply = await this.TOMB.totalSupply();
    const tombRewardPoolSupply = await this.TOMB.balanceOf(PhoenixAvaxRewardPool.address);
  ...
  async getLPStat(name: string): Promise<LPStat> {
    ...
    const token0 = name.startsWith('PHOENIX') ? this.TOMB : this.TSHARE;
    const isTomb = name.startsWith('PHOENIX');
  ...
  async getShareStat(): Promise<TokenStat> {
    const { PhoenixAvaxLPTShareRewardPool } = this.contracts;

    const supply = await this.TSHARE.totalSupply();
    const priceInAVAX = await this.getTokenPriceFromPancakeswap(this.TSHARE);
    const tombRewardPoolSupply = await this.TSHARE.balanceOf(PhoenixAvaxLPTShareRewardPool.address);
  ...
  async getTombStatInEstimatedTWAP(): Promise<TokenStat> {
    const { SeigniorageOracle, PhoenixAvaxRewardPool } = this.contracts;
  ...
  async getGravePriceInLastTWAP(): Promise<BigNumber> {
    const { Treasury } = this.contracts;
    return Treasury.getPhoenixUpdatedPrice();
  }

  async getBondsPurchasable(): Promise<BigNumber> {
    const { Treasury } = this.contracts;
    return Treasury.getBurnablePhoenixLeft();
  }
  ...
  async getTokenPerSecond(
    ...
    if (depositTokenName.startsWith('PHOENIX-AVAX')) {
      return rewardPerSecond.mul(30000).div(59500);
    } else if (depositTokenName.startsWith('XSHARE-AVAX')) {
  ...
  async earnedFromBank(
    ...
    try {
      if (earnTokenName === 'PHOENIX') {
        // problem is pendingPHOENIX isnt a function since the abi still says pendingTOMB
        return await pool.pendingPHOENIX(poolId, account);
      } else {
  ...
  async quoteFromSpooky(tokenAmount: string, tokenName: string): Promise<string> {
    const { JoeRouter } = this.contracts;
    const { _reserve0, _reserve1 } = await this.TOMBWAVAX_LP.getReserves();
    let quote;
    if (tokenName === 'TOMB') {
      quote = await JoeRouter.quote(parseUnits(tokenAmount), _reserve1, _reserve0);
    } else {
      quote = await JoeRouter.quote(parseUnits(tokenAmount), _reserve0, _reserve1);
    }
